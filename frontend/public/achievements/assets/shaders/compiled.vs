...
void main() {
    ...
    hsl.x = baseHsv.x - length(vUv-0.5) * 0.2;
    hsl.y = baseHsv.y * hsl.y * 0.5;
...
void main() {
    ...
    // Grayscale conversion and tinting to avoid mixing blue with uColor
    float gray = dot(color, vec3(0.299, 0.587, 0.114));
    color = vec3(gray) * uColor;
...
float logo_sdf(vec3 p) {
    // 1. Flip orientation (Blender Z-up to WebGL Y-up) and center
    // The S was inverted, so we flip the Y axis.
    vec3 q = vec3(p.x, -p.y, p.z); 
    q.y -= 0.05; // Slight vertical offset
    
    // 2. Sharp "S" Math (Analytical)
    float sz = 0.45; // Scale
    float th = 0.08; // Thickness
    
    // Top and Bottom loops
    float d1 = length(q.xy - vec2(0.0, sz*0.5)) - sz*0.5;
    float d2 = length(q.xy - vec2(0.0, -sz*0.5)) - sz*0.5;
    
    // Masking to create the 'S' shape
    float s = q.x > 0.0 ? d1 : d2;
    if (q.y > sz*0.5 && q.x < 0.0) s = length(q.xy - vec2(0.0, sz*0.5)) - sz*0.5;
    if (q.y < -sz*0.5 && q.x > 0.0) s = length(q.xy - vec2(0.0, -sz*0.5)) - sz*0.5;
    ...
    // 3. Extrude to 3D
    float depth = 0.1;
    float w = abs(q.z) - depth;
    return (min(max(d, w), 0.0) + length(max(vec2(d, w), 0.0))) * 0.8;
}
...
vec3 getFBR(vec3 baseColor, vec2 uv, vec3 normal) {
    ...
    return (((baseColor * uColor) * texture2D(tMatcap, mUV).rgb) + color) * mro.b;
}
