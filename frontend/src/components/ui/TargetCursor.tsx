...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  const isMobile = useMemo(() => {
    ...
    return mobileRegex.test(userAgent.toLowerCase()) && window.innerWidth <= 1024;
  }, []);
  ...
  useEffect(() => {
    if (isMobile) {
      console.log("TargetCursor: Disabled on mobile");
      return;
    }
    if (!cursorRef.current) {
      console.log("TargetCursor: No cursor ref");
      return;
    }

    console.log("TargetCursor: Initializing");
  ...
  return (
    ...
    >
      <div
        ref={dotRef}
        className="absolute top-1/2 left-1/2 w-1.5 h-1.5 bg-primary rounded-full -translate-x-1/2 -translate-y-1/2 shadow-[0_0_10px_var(--primary)]"
        style={{ swallowTouch: "none", willChange: "transform" } as any}
      />
      <div
        className="target-cursor-corner absolute top-1/2 left-1/2 w-4 h-4 border-[2px] border-primary -translate-x-[150%] -translate-y-[150%] border-r-0 border-b-0"
        style={{ willChange: "transform" }}
      />
      <div
        className="target-cursor-corner absolute top-1/2 left-1/2 w-4 h-4 border-[2px] border-primary translate-x-1/2 -translate-y-[150%] border-l-0 border-b-0"
        style={{ willChange: "transform" }}
      />
      <div
        className="target-cursor-corner absolute top-1/2 left-1/2 w-4 h-4 border-[2px] border-primary translate-x-1/2 translate-y-1/2 border-l-0 border-t-0"
        style={{ willChange: "transform" }}
      />
      <div
        className="target-cursor-corner absolute top-1/2 left-1/2 w-4 h-4 border-[2px] border-primary -translate-x-[150%] translate-y-1/2 border-r-0 border-t-0"
        style={{ willChange: "transform" }}
      />
    </div>
  );
