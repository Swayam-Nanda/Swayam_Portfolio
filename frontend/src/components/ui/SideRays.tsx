...
interface SideRaysProps {
  ...
  intensity?: number;
  spread?: number;
  rayPos?: { x: number; y: number }; // Dynamic position [0-1]
  tilt?: number;
  saturation?: number;
  blend?: number;
  falloff?: number;
...
void main() {
  ...
  vec2 rayPos = vec2(iRayPos.x * iResolution.x, iRayPos.y * iResolution.y);
  ...
  gl_FragColor = color;
      ...
      const uniforms = {
        ...
        iRayPos: { value: [rayPos.x, rayPos.y] }
      };
  ...
  useEffect(() => {
    ...
    u.iRayPos.value = [rayPos.x, rayPos.y];
  }, [speed, rayColor1, rayColor2, intensity, spread, rayPos.x, rayPos.y, tilt, saturation, blend, falloff, opacity]);
